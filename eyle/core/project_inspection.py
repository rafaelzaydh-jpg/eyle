...
def _safe_read(root: str, rel: str, max_bytes: int) -> Tuple[str | None, int, str | None]:
...
def project_stats(root: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for rel in collected["files"]:
        ...
        text, size, error = _safe_read(root, rel, max_bytes)
    ...
    return {
        ...
        "skipped": {"file_too_large": skipped_large, "read_error": skipped_read},
...
def count_tokens(root: str, config: Dict[str, Any], path: str | None = None, tokenizer: str | None = None) -> Dict[str, Any]:
    ...
    chars = lines = bytes_total = measured = skipped_large = skipped_read = 0
    for rel in files:
        text, size, error = _safe_read(root, rel, max_bytes)
        if error == "file_too_large":
            skipped_large += 1
