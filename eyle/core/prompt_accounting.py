...
_COMPONENT_GROUPS = {
    ...
    "evidence_state": {"investigation", "observation_map", "evidence_index"},
...
def build_prompt_cost_accounting(details: Dict[str, Any], *, limit: int = 20) -> Dict[str, Any]:
    ...
    for logical in logical_calls:
        ...
        for attempt in attempts:
            ...
            for key in ("selected_evidence_count", "evidence_excerpt_chars_per_item", "answer_anchor_count", "investigation_target_count", "prompt_budget_tokens"):
    ...
    diagnostics={"observation_count":observation_count,"evidence_count":evidence_count,"observation_replays":replays,"tool_requests_observed":tool_requests}
    amplification=_round_ratio(float(evidence_count),float(observation_count))
    if amplification is not None: diagnostics["evidence_amplification_ratio"]=amplification
    ...
    for key in ("target_attached_evidence_count","claim_cited_evidence_count","structurally_unreferenced_evidence_count","structurally_unreferenced_tool_actions"):
