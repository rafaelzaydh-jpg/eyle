...
def _source_files(root: str, *, max_files: int | None, max_bytes: int) -> Tuple[List[str], bool, int]:
    ...
    for current, dirs, names in os.walk(root, followlinks=False):
        ...
        for name in sorted(names):
            ...
            if is_protected_workspace_resource(root, relpath, index=protected_index):
                protected_skipped += 1
                continue
            try:
...
def analyze_symbol_relations(
    ...
    files, file_truncated, protected_skipped = _source_files(root, max_files=max_files, max_bytes=max_file_bytes)
    ...
    if query == "reachability":
        ...
        if not shortest and not definitions:
            ...
            if protected_skipped:
                frontiers.append({
                    "kind": "protected_resource_boundary", "at": "project_scan",
                    "reason": "one or more source-like resources were excluded because they resolve to protected physical resources",
                    "count": protected_skipped, "expandable": False,
                })
