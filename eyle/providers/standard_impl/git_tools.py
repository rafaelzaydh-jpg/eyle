...
def git_status(root: str, max_entries: int = 200) -> Dict[str, Any]:
    ...
    for raw in lines:
        ...
        counts[category] += 1
        item = {"path": normalized, "status": code, "category": category}
        if is_protected_workspace_resource(root, normalized, index=protected_index):
...
def git_diff(
    ...
    *,
    ...
    if path:
        ...
        if is_protected_workspace_resource(root, normalized_path, index=protected_index):
    ...
    protected_paths = [
        ...
        if is_protected_workspace_resource(root, str(item.get("path") or ""), index=protected_index)
    ]
    if protected_paths and not normalized_path:
        diff_args = [*base, f"--unified={context_lines}", "--", "."]
        diff_args.extend(f":(exclude){path}" for path in protected_paths)
    ...
    return {
        ...
        "files": [dict(item, **({"content_access": "protected"} if is_protected_workspace_resource(root, str(item.get("path") or ""), index=protected_index) else {})) for item in files],
        "file_count": len(files),
        "protected_resources_omitted": len(protected_paths),
        "coverage_scope": "readable_changed_files" if protected_paths else "all_changed_files",
