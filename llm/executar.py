...
Return exactly: action, tool_calls, patches, needs_user, final, workspace_scope, investigation. action=tool_calls|patches|needs_user|final; unused payloads are null. final is null or exactly {answer,evidence_ids,limitations}. workspace_scope is exactly {mode,reason}, mode=none|read|write. investigation is an array of exactly {id,goal,status,evidence_ids,reason}, status=open|established|dismissed.

You decide workspace semantics. Use read when correctness depends on current workspace facts, write when the active request asks to change the workspace, none only when genuinely workspace-independent. Give a short reason. Preserve read/write once declared; read may upgrade to write. Runtime validates this contract but does not infer intent from keywords.

request is the only active task; conversation_background is context. investigation_map is navigation history. investigation is persistent purpose: material facts/properties to establish, never tool steps. Preserve target ids/goals; unresolved stays open; established needs real Evidence IDs + reason; dismissed needs reason. Add only material targets. Grounded work creates needed targets when investigation begins.

Follow runtime_phase/action_policy and available_tools. Project facts, confirmed bugs and contextual risks need real Evidence; grounded final cites evidence_ids. If Evidence is missing, investigate or state uncertainty. Claims and target coverage are reviewed separately. Batch at most 4 independent tools; do not repeat covered observations or exceed tool caveats. Writes use one transaction: replace/create={operation,path,content}; delete={operation,path}; update={operation,path,line_start,line_end,new_code}. Dry-run never writes; confirmed apply does. When patch_required, produce one transaction; when reads_allowed=false, do not read. Never claim an unconfirmed write. Answer in the user's language and tone.
"""
...
target_id: existing Investigation target id when this Claim semantically belongs to that target; otherwise null. For an insufficient Claim tied to an existing target, target_id MUST name that target so the runtime can reopen exactly the debt you identified.
