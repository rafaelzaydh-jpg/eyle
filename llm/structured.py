...
_AGENT_SCHEMA = {
    ...
    "properties": {
        ...
        "final": {"anyOf": [{"type": "null"}, {"type": "string"}, {"type": "object"}]},
        "plan": {"type": "array", "items": {"type": "string"}},
    },
    "required": ["action", "tool_calls", "patches", "needs_user", "final", "plan"],
    "additionalProperties": False,
}
    ...
    "properties": {
        "claims": {
            ...
            "items": {
                ...
                "required": ["id", "answer_ref", "statement", "kind", "evidence_ids", "verdict", "reason"],
                "additionalProperties": False,
            },
        },
        "findings": {
        ...
        "semantic_gaps": {
            ...
            "items": {
                ...
                "required": ["id", "type", "evidence_ids", "reason"],
                "additionalProperties": False,
            },
        },
    },
...
_PROFILE_TOP_LEVEL = {
    "agent": ("action", "tool_calls", "patches", "needs_user", "final", "plan"),
...
def parse_agent_response(raw: Any) -> Dict[str, Any]:
    ...
    if not isinstance(value.get("plan"), list) or any(not isinstance(item, str) for item in value.get("plan")):
        raise StructuredResponseError("AGENT_PLAN_INVALID", "plan must be an array of strings")
    ...
    if action == "tool_calls":
        ...
        return {"tool_calls": normalized, "plan": value["plan"]}
    if action == "patches":
        ...
        return {"patches": normalized_patches, "plan": value["plan"]}
    if action == "needs_user":
        ...
        return {"needs_user": question.strip(), "plan": value["plan"]}
    ...
    return {"final": final, "plan": value["plan"]}
...
def parse_claim_review_response(raw: Any) -> Dict[str, Any]:
    ...
    claim_keys = {"id", "answer_ref", "statement", "kind", "evidence_ids", "verdict", "reason"}
    ...
    gap_keys = {"id", "type", "evidence_ids", "reason"}
    for index, item in enumerate(value["semantic_gaps"], start=1):
        item = _exact_item(item, gap_keys, code="CLAIM_REVIEW_SEMANTIC_GAP_SHAPE_INVALID", detail=f"semantic_gaps[{index}] must contain exactly id, type, evidence_ids and reason")
