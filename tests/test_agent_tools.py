...
def test_live_read_tools_share_envelope(tmp_path):
    ...
    calls=[('standard.list_tree',{}),('standard.search_code',{'query':'return 1'}),('standard.find_symbol',{'symbol':'x'}),('standard.read_file',{'path':'a.py'}),('standard.read_file',{'path':'a.py','line_start':1,'line_end':2})]
...
def test_search_code_reads_live_workspace_without_index(tmp_path):
    ...
    result=standard_registry().execute('standard.search_code',{'query':'volume'},ctx(tmp_path))
...
def test_validation_rejects_unknown_and_bad_arguments(tmp_path):
    bad=standard_registry().execute('standard.read_file',{'path':'a.py','line_start':'1','line_end':2},ctx(tmp_path))
