...
def test_live_read_tools_share_envelope(tmp_path):
    ...
    calls=[('list_tree',{}),('search_code',{'query':'return 1'}),('find_symbol',{'symbol':'x'}),('read_file',{'path':'a.py'}),('read_range',{'path':'a.py','line_start':1,'line_end':2})]
...
def test_validation_rejects_unknown_and_bad_arguments(tmp_path):
    bad=tools.executar_tool('read_range',{'path':'a.py','line_start':'1','line_end':2},ctx(tmp_path))
    assert bad['error_code']=='INVALID_ARGUMENT' and bad['executed'] is False
    missing=tools.executar_tool('read_range',{'path':'a.py','line_start':1},ctx(tmp_path))
