...
def test_agent_can_choose_execution_trace_and_cite_it(monkeypatch, tmp_path):
    ...
    def fake(prompt, cfg):
        ...
        evidence_id = result["source_record_ids"][0]
        return {"final": {
            "answer": "O trace atual registra a composição do contexto sem expor o prompt bruto.",
            "limitations": [],
            "evidence_ids": [evidence_id],
        },
        "investigation_updates": [{"id": "T1", "goal": "Establish what the current trace records", "status": "established", "evidence_ids": [evidence_id], "reason": "The trace SourceRecord was explicitly admitted."}]}
