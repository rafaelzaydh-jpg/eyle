...
def test_rev372_config_accepts_only_current_identity():
    ...
    current = base_config()
    assert validar_config(copy.deepcopy(current), registry)["revision"] == "rev3.7.2-ecc"
...
def test_rev372_session_and_pending_are_current_schema_only():
    ...
    assert session["session_schema_version"] == SESSION_SCHEMA_VERSION == "2.7.5-r3.7.2-ecc"
