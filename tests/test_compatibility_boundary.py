...
def test_search_code_backends_share_one_canonical_order_and_truncation(monkeypatch, tmp_path):
    ...
    files = tools._searchable_files(str(tmp_path))

    fallback = tools._search_matches_fallback(str(tmp_path), "needle", files, 2)
    raw = [
    ...
    ripgrep = tools._search_matches_with_rg(str(tmp_path), "needle", files, 2)

    assert ripgrep == fallback
    matches, observed, truncated = ripgrep
    assert [item["file"] for item in matches] == ["app.py", "docs.md"]
    assert observed == 3
