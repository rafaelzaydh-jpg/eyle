...
{
  "app_version": "2.7.4",
  "config_schema_version": "5.8",
  "revision": "rev5.8-objective-projection-evidence-admission"
}
```
...
The sandbox may create/delete files, install packages/toolchains, compile and run arbitrary commands without confirmation. In Docker, both snapshot changes and container root-filesystem changes persist across `run_command` calls in the same job. Docker may auto-pull the configured base image when absent. Protected credential/private-key resources and their physical aliases are omitted from the snapshot and the real workspace is never mounted read-write. Rev5.8 performs no content-based secret classification on normal source files. A real workspace change still requires the canonical confirmed `WriteTransaction`.
...
Rev5.8 uses one canonical English contract inside the Core. Sandbox backend values are exactly:
