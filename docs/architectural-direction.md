...
## Scaling criterion: do not buy efficiency by hiding reality

The long-term target is not “minimum tokens at any cost.” It is to make LLM work track semantic difficulty while deterministic capability work absorbs growth in the external state space.

```text
LLM cost     ≈ semantic complexity + material observation
machine cost ≈ size of repository / network / device set / data space
```

Capabilities may exhaustively compute an **objectively specified** property over arbitrarily large state and may deterministically compact/group/page the result. They must not decide which facts are semantically relevant, sufficient or material to the user's objective. Those decisions belong to Main/Claim.

A bounded projection is acceptable only when its incompleteness is explicit and the remaining objective state stays addressable when the capability can continue. This principle is domain-neutral and is one of the mechanisms by which the same Core could eventually operate code, networks, cloud resources, devices or other domains without giving each capability its own semantic brain.
...
Rev5.8 intentionally keeps Agent and Claim on one strict JSON-Schema contract. A future LLM adapter layer may support providers that expose structured output through different native mechanisms, for example:
...
The exact schema should be proven by multiple capabilities before being frozen. Current Rev5.8 code-reachability fields such as `files_scanned`, `roots_tested`, `shortest_path_hops` and `objective_result=reachable` are **domain payload**, not universal Coverage semantics.
...
None of these are current Rev5.8 product capabilities. They are examples validating why the observation protocol should remain domain-neutral.
