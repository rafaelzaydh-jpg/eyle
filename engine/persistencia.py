...
def salvar_jsonl_atomico(caminho, itens):
    """Publica uma sequencia JSONL inteira sem expor arquivo parcial."""
    def escrever(arquivo):
        for item in itens:
            arquivo.write(json.dumps(item, ensure_ascii=False) + "\n")

    _publicar_atomico(caminho, escrever)


def salvar_texto_atomico(caminho, texto):
    _publicar_atomico(caminho, lambda arquivo: arquivo.write(str(texto)))
