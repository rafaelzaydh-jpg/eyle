...
possivel, com seguranca:
    ...
    Proposta (gerada em engine/engine.py, LLM Engenheiro)
        ...
        -> Aplicar          (aplicar_patch: SO chamado depois que o
                             ...
                             engine/engine.py:_aplicar_proposta_pendente.
...
decisao de quem chama (engine/engine.py), que so chama aplicar_patch()
...
from ingest import extrair_definicoes_python, extrair_simbolos  # mesmo indice do ingest
from engine.seguranca import _resolver_caminho_seguro
from engine.text_hash import hash_faixa as _hash_faixa_canonica, hash_texto as _hash_texto_canonico
from engine.sandbox import executar_no_sandbox
from engine.retencao import limpar_backups
...
def localizar_simbolo(caminho_projeto, caminho_relativo, simbolo):
    """
    Le o ARQUIVO REAL (fresco do disco, NAO a memoria indexada -- o
    arquivo pode ter mudado desde o ultimo ingest) e localiza a linha
    exata onde `simbolo` (funcao/classe) comeca e termina, reaproveitando
    o mesmo reconhecedor de definicoes do ingest.py. Python usa as posicoes
    ...
    desde o ultimo ingest) -- nunca inventa uma posicao.
    """
    ...
    if ext == ".py":
        ...
        encontrados = [
            d for d in extrair_definicoes_python(linhas)
            if d["nome"] == simbolo
        ]
...
def calcular_impacto(arquivo_alvo, entendimento):
    """
    'depende_de invertido': quais arquivos do Modelo Interno
    (entendimento.json['arquivos']) declaram `arquivo_alvo` na propria
    lista `depende_de` -- ou seja, quem quebra se a interface/comportamento
    de `arquivo_alvo` mudar. Nao precisa de indice persistido: o Modelo
    Interno inteiro ja cabe em memoria, entao um loop simples resolve.

    Devolve lista de {"arquivo", "responsabilidade"}.
    """
    arquivos = (entendimento or {}).get("arquivos", {})
    impactados = []
    for caminho, info in arquivos.items():
        if arquivo_alvo in (info.get("depende_de") or []):
            impactados.append({
                "arquivo": caminho,
...
def aplicar_patch(caminho_projeto, caminho_relativo, linha_inicio, linha_fim,
    ...
    desde a proposta (edicao manual do usuario, outro patch, novo ingest
